...
class PreProcessing:
    ...
    def __init__(self, data_src, neg_data_src):
        self.data_src = data_src
        self.neg_data_src = neg_data_src
        self.images_train, self.images_test, self.labels_train, self.labels_test = self.preprocessing(1)
    ...
    def get_triplets(self):
        label_ap, label_n = np.random.choice(self.unique_train_label, 2, replace=False)
        # a, p = np.random.choice(self.map_train_label_indices[label_ap],2, replace=False)
        a, p = np.random.choice(self.map_train_label_indices[label_ap],2)
    ...
    def generate_triplets(self):
        triplets = []
        a, p, n = self.get_triplets()
        return self.images_train[a], self.images_train[p], self.images_train[n]